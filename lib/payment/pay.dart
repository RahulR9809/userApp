...
class _WebViewScreenState extends State<WebViewScreen> {
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          onNavigationRequest: (NavigationRequest request) {
            ...
            // Block any URL that is not the initial checkout URL
            if (request.url != widget.checkoutUrl) {
              // Prevent navigation and display a dialog instead
              _showPaymentDialog(success: request.url.contains('payment-success'));
              return NavigationDecision.prevent;
            }
            return NavigationDecision.navigate; // Allow navigation for the checkout URL
          },
        ),
      )
  ...
  void _showPaymentDialog({required bool success}) {
    ...
    showDialog(
      ...
      context,
      MaterialPageRoute(
        builder: (context) => PaymentCompletedPage(), // Replace with your desired page
      ),
    );
            }, // Close the dialog
            child: const Text('OK'),
