...
// void _onToggleBottomBar(ToggleBottomBar event, Emitter<RideState> emit) {
// print(' the bottom bar visiblity $_isBottomBarVisible');
//     _isBottomBarVisible = !_isBottomBarVisible;
//     emit(BottomBarVisibilityChanged(isVisible: _isBottomBarVisible));
...
// void _onUpdateCurrentLocation(
//     UpdateCurrentLocation event, Emitter<RideState> emit) {
//   print('Event received: UpdateCurrentLocation');
//   print('Latitude: ${event.latitude}, Longitude: ${event.longitude}, Address: ${event.address}');
  
//   emit(LocationLoaded(event.latitude, event.longitude, event.address));
//   print('State emitted: LocationLoaded');
// }


//   Future<void> _onFetchSuggestions(FetchSuggestions event, Emitter<RideState> emit) async {
//     emit(SuggestionsLoading());
//     try {
//       final suggestions = await RideService().fetchPickupLocation(event.query);
//       emit(SuggestionsLoaded(suggestions));
//     } catch (e) {
//       emit(DestinationError('Failed to fetch suggestions: $e'));
//     }
//   }

//   Future<void> _onSelectSuggestion(SelectSuggestion event, Emitter<RideState> emit) async {
//     try {
//       final locations = await locationFromAddress(event.address).timeout(
//         const Duration(seconds: 4),
//       );
...
//       }
//     } catch (e) {
//       emit(DestinationError('Failed to select address: $e'));
//     }
//   }

//   Future<void> _onUpdateDestination(UpdateDestination event, Emitter<RideState> emit) async {
//     emit(RideLocationUpdated(
//       event.latitude,
//       event.longitude,
//       event.address,
//     ));
...
//         paymentMethod: event.paymentMethod,
//       );
// print(event.pickupLocation);
// print(event.dropLocation);
//       if (response.isNotEmpty) {
...
//   FutureOr<void> _onSelectDestination(SelectDestination event, Emitter<RideState> emit) {
//     emit(AddressSelected(event.latitude, event.longitude, event.address));
//   }
// }
